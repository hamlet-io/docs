...
```bash
cookiecutter /opt/`hamlet`/patterns/cmdb/tenant
```
...
```bash
cookiecutter /opt/`hamlet`/patterns/cmdb/account
```
...
```bash
cookiecutter /opt/`hamlet`/patterns/products/app-lifecylce-mgmt
```
...
- `/var/opt/`hamlet`/automation/config/solutionsv2/shared/default/solution.json` - The solution shows you the infrastructure components that will be deployed to create your product
- `/var/opt/`hamlet`/automation/config/solutionsv2/alm/default/jenkins/settings.json` - Contains the environment specific settings for your Jenkins deployment
...
Edit the domains.json file `/var/opt/`hamlet`/accounts/<tenantname>/domains.json`
