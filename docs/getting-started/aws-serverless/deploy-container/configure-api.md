...
With hamlet we can defined environment variables using settings, settings are a namespaced configuration object that can be shared across your solution components and can include files or complex structures that will be converted to environment variables as required.

So lets set the environment variable `LOCATION` to work
...
1. The next step is to find the settings namespace for our api component. Components are configured with setting namespaces that are merged to form the settings for a component. The default namespaces are based on the name of the components and the deployment unit the component belongs to and you can create your own namespaces if you need to.
    ...
    First, lets find the name of our api occurrence, this is its unique reference within the solution
...
1. The helloapi service is the one that we want to find the settings for, so the occurrence we are looking for is application-ecshost-helloapi-service. To find the setting-namespace we add a query argument to our description
    ...
    Looking through this list you can see the default namespaces, all of them are based on details of the occurrence and where it is hosted. These vaules are similar to the link configuration that we looked at before.

    To keep it consistent with what we've configured in the solution we will use the Id based namespace, `app-ecshost-helloapi`
...
1. Now that we have the directory lets add the settings

    From you editor add a file called `settings.json` in the setting namespace repository and add the following
...
1. Change back to our solutions directory to run the deployment
...
1. Now we can generate the deployment, this time we will have a look at what will change before the deployment is run
    ...
    In this changeset the TaskDefinition properties have been updated. We can't see exactly what changes have been made though. Let's check the generated infrastructure template.

    Press `n` on the confirmation prompt so that we can confirm the new environment variable has been set

1. Hamlet saves the content of the CloudFormation template that it generates as part of the CMDB, this is really useful for following the history of the deployment if this repo is part of source control or you want to troubleshoot the deployment.
...
1. Let's have a look at whats in the directory. We will find a collection of json and sh scripts which are used to deploy the helloapi
...
1. Now that we know the variable has been set lets run the deployment
...
1. Repeating what we did in the API deployment we will get the URL for the load balancer to check that the location has changed
    ...
    Now our API is saying hello from the work

So this guide has covered how you can include application level configuration into your solution and how it is applied to your occurrences.
