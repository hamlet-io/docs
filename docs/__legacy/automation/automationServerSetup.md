---
sidebar_label: app lifecycle setup
title: Application Lifecycle Management Setup
---
...
```text
<Environment name="JENKINS_HOME" value="/`hamlet`/jenkins/" type="java.lang.String"/>
```
...
mkdir -p /`hamlet`/jenkins
chown tomcat:tomcat /`hamlet`/jenkins
```
...
```bash
cp -rp /var/lib/docker /`hamlet`
rm -rf /var/lib/docker
ln -s /`hamlet`/docker /var/lib/docker
...
services:
  ...
  postgres:
    ...
    env_file: sentry.env
    volumes:
      - /`hamlet`/sentry/var/pg_data/:/var/lib/postgresql/data
      - /`hamlet`/sentry/var/pg_backups/:/backups
...
Go to Jenkins at https://automation.{domain} in a browser (initial admin password in `/`hamlet`/jenkins/secrets/initialAdminPassword`).
