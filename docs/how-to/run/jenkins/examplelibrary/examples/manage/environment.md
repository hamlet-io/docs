...
This pipeline's available parameter values are populated by the [properties](../properties) file and so initially, no parameters can be provided. After loading the `properties` file the pipeline will prompt for user selection of the following parameters, which will each filter the pipelines scope:
...
Loads the [properties](../properties) file and prompts for the selection of the now-configured Parameters.
