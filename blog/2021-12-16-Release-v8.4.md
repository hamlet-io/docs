...
For more details on using settings see the [settings how to guide](docs/how-to/define/adding-env-vars)
...
The tasks are defined as part of the hamlet engine and describe the parameters for each task along with a description of what should be completed. The executor is then responsible for implementing these tasks inline with the description.
...
- dos2unix has been removed from bash scripts, if this is required it will need to managed outside of hamlet from now on. Git mostly handles this process anyway
...
- If your are using the URL or containerregsitry image sources you can skip pulling the image by setting the env var `HAMLET_SKIP_IMAGE_PULL=true` when running a deployment. This is handy when you are verifying a deployment
...
- When deploying a directory component a route53 resolver is added to route DNS requests from the standard AWS DNS server to the AD servers. This removes the need for creating DHCP chagnes or manually configuring your DNS servers on instances
...
The Azure plguin has had some updates focussing on Networking and directories
